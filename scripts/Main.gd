...
func _ready() -> void:
	GameManager.connect("mode_changed", Callable(self, "_on_mode_changed"))
	# Boot into LoneWolfNarrative automatically on startup
	_load_mode_scene(GameManager.GameMode.LONE_WOLF_NARRATIVE)

func _on_mode_changed(new_mode: GameManager.GameMode) -> void:
	_load_mode_scene(new_mode)

func _load_mode_scene(mode: GameManager.GameMode) -> void:
	# Clean up active node
	if is_instance_valid(active_mode_instance):
		active_mode_instance.queue_free()
		active_mode_instance = null

	var scene_path: String = GameManager.MODE_SCENES.get(mode, "")
