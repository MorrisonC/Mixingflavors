...
- Name: graphics.spec.js >> Graphics QA - Visual Regression >> Level Select Snapshot - Ultrawide
- Location: tests\playwright\graphics.spec.js:93:5

# Error details

```
Error: A snapshot doesn't exist at C:\Development\Mixingflavors\tests\playwright\graphics.spec.js-snapshots\level-select-ultrawide-chromium-win32.png, writing actual.
```
...
```ts
  ...
  2   | 
  3   | test.describe('Graphics QA - Visual Regression', () => {
  4   |   let webGLWarnings = [];
  5   |   let consoleErrors = [];
  6   | 
  ...
  14  |         consoleErrors.push(text);
  15  |       }
  16  |       if (text.includes('WebGL') || text.includes('shader')) {
  17  |         webGLWarnings.push(text);
  18  |       }
  19  |     });
  20  | 
